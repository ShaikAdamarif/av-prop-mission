...
function renderAdminTab(t){
  ...
  if(t==='overview'){
    ...
    c.innerHTML=subs.length?subs.map(s=>`<div class="list-row"><div><b>${esc(s.title)}</b> <span class="tag ${s.status==='done'?'success':'pending'}">${s.status==='done'?'Success ✓':'Pending'}</span>
      ...
      <button class="btn ghost" onclick="window._av.editSub('${s.id}')">Edit</button>
      <button class="btn danger" onclick="window._av.delSub('${s.id}')">Delete</button>
...
_av.editSub = (id) => {
  const s=db.get(KEYS.submissions,[]).find(x=>x.id===id);
  if(!s) return toast('Not found');
  modal(`<h2 class="section-title">Edit Submission</h2>
    <div class="input-group"><label>Title</label><input id="es_t" value="${esc(s.title||'')}"/></div>
    <div class="input-group"><label>Description</label><textarea id="es_d" rows="4">${esc(s.desc||'')}</textarea></div>
    <div class="input-group"><label>Status</label>
      <select id="es_s">
        <option value="pending" ${s.status!=='done'?'selected':''}>Pending</option>
        <option value="done" ${s.status==='done'?'selected':''}>Done</option>
      </select>
    </div>
    <button class="btn" onclick="window._av.saveSub('${id}')">Save</button>`);
};
_av.saveSub = (id) => {
  const list=db.get(KEYS.submissions,[]);const s=list.find(x=>x.id===id);if(!s) return;
  s.title=document.getElementById('es_t').value;
  s.desc=document.getElementById('es_d').value;
  const newStatus=document.getElementById('es_s').value;
  if(newStatus==='done' && s.status!=='done') s.completedAt=now();
  s.status=newStatus;
  db.set(KEYS.submissions,list);
  closeModal();toast('Saved');renderAdminTab('submissions');
};
_av.delSub = (id) => {
  if(!confirm('Delete this submission permanently?')) return;
  let list=db.get(KEYS.submissions,[]);
  list=list.filter(x=>x.id!==id);
  db.set(KEYS.submissions,list);
  toast('Deleted');renderAdminTab('submissions');
};
_av.sendOffer = async () => {
